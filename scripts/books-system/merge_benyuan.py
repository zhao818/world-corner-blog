# -*- coding: utf-8 -*-
"""《信任本源论》正文 → 成书 markdown

读 I:\灵魂之觅\docs\信任本源论.md(定稿全文),做机械整理:
  - 剔除书首 # 书名行(书名由听书页 yaml / 文章页提供)
  - 剔除书首文档头(> 引用块:定位/关系/写作规矩,均为元信息不朗读)
  - 剔除文末 *初稿于...* / *关联:* 两行斜体元信息
  - 剔除文末 <!-- 内部导航:... --> HTML 注释行(含知识库内部路径,必删)
  - ## 章标题 → # 一级(TTS split 按 # 一级切章;朗读用 qunti 式 title 注入)
  - 其余正文照抄(含 --- 分隔线、正文 > 引言、**加粗**)
输出 books-system/信任本源论-灵魂之觅版.md。

注:剔除谓词扩到 *初稿于*(merge_xinqian 只认 *成稿于);HTML 注释按行首 <!-- 剔除。

用法:python merge_benyuan.py
"""
import os
import re

SRC = r"I:\灵魂之觅\docs\信任本源论.md"
BASE = os.path.dirname(os.path.abspath(__file__))
DST = os.path.join(BASE, "信任本源论-灵魂之觅版.md")

EXPECT_CHAPTERS = 7  # 〇开卷/一~五/尾声
