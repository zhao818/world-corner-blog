...
克隆 tts_lunli.py 的清洗、合成、合并逻辑,针对本文调整:
  ...
  合并   段间/节间 400ms 静音, ffmpeg concat -c copy → static/audio/buyi.mp3
...
SRC = os.path.join(BASE, "信任伦理篇补遗-灵魂之觅版.md")
OUT_TMP = r"F:\xiaochenxukaifa\有声书-伦理篇补遗\成品\_chapters"
FINAL = os.path.normpath(os.path.join(BASE, "..", "..", "static", "audio", "buyi.mp3"))
...
# 节名(仅用于时间戳/展示;朗读用一级标题原文,见 main 中 title 注入;与 audiobook.yaml chapters 同名保持一致)
CHAPTER_NAMES = [
