# -*- coding: utf-8 -*-
"""《信任·伦理篇》6 章 → 单文件有声书 mp3(edge-tts 晓晓女声, +2%)

复用 tts_benyuan.py 的清洗、合成、合并逻辑,针对本文调整:
  章节   6 章,一级标题也朗读(本文是单篇长文,听众需要章边界,title 注入)
  表格   本文无 markdown 表格(TABLE_SPEECH 留空兜底)
  符号   箭头 → 读「到」;乘号 × → 顿号(正文虽未见 ×,保留兜底)
  合成   edge-tts zh-CN-XiaoxiaoNeural, rate +2%, 3000 字/段, 6 并发, 失败重试 3 次
  合并   段间/章间 400ms 静音, ffmpeg concat -c copy → static/audio/lunli.mp3
  时间   用 ffprobe 测每章时长,输出每章起始秒数供 audiobook.yaml chapters

用法:
    python tts_lunli.py           # 合成全部 6 章并合并(已存在则跳过)
    python tts_lunli.py 1         # 只合成第 1 章(冒烟),不合并 final
"""
import asyncio
import os
import re
...
SRC = os.path.join(BASE, "信任伦理篇-灵魂之觅版.md")
OUT_TMP = r"F:\xiaochenxukaifa\有声书-伦理篇\成品\_chapters"
FINAL = os.path.normpath(os.path.join(BASE, "..", "..", "static", "audio", "lunli.mp3"))
...
def main():
    ...
    assert len(chapters) == 6, "章数 %d != 6" % len(chapters)
