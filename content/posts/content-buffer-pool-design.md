...
Gmail (过滤后的邮件)
    │
Telegram (收藏/夹的消息) ──► 脚本聚合 ──► ~/buffer/raw/  ──► 定时处理 ──► ~/buffer/processed/
    │                              │
RSS (过滤后的文章)             过滤+去重           标记+分类
```
...
**认知税节省**：每天少刷 3 次邮箱 × 5 分钟 = 15 分钟。一年 91 小时。
...
**认知税节省**：在 Telegram 的碎片讨论和深度文章之间加了一层过滤器。只让有价值的内容进入缓冲池。
